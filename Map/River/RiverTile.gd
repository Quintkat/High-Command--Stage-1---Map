...
func _ready():
	subtileWidth = TileConstants.SIZE/3
...
func addDir(dir : String, size : String):
	...
	node.visible = true
	node.scale[0] = Rivers.getWidthFactor(size)
	dirSizes[dir] = size
	resolveJoint()
	...
	if len(dirsVisible) == 2:
		...
		for dir in dirsVisible:
			if dir in HORIZONTAL:
				RiverJoint.scale[1] = Rivers.getWidthFactor(dirSizes[dir])
			
			if dir in VERTICAL:
				RiverJoint.scale[0] = Rivers.getWidthFactor(dirSizes[dir])
	else:
		RiverJoint.visible = false
